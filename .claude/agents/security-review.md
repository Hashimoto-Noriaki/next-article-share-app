...
CLAUDE.md の方針により、**認証・認可・入力検証・機密情報の露出はすべて Critical** とする。

- **Critical**: 実際に悪用できる経路がある（未認証アクセス、他人のリソース操作、未検証入力、秘密情報の露出、XSS、生 SQL）
- **Warning**: 現時点で悪用経路は確認できないが防御が 1 層欠けている（例: handler では所有者チェックがあるが repository の `where` に所有者条件がない）
